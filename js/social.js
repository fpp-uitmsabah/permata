/**
 * Social Media Features Module
 * Provides like, comment, follow, and share functionality for faculty portfolios
 */
...
const SocialAPI = {
    ...
    async like(facultyId, reactionType = 'like') {
        ...
        const response = await fetch(`${API_BASE_URL}/social/like`, {
            ...
            body: JSON.stringify({ facultyId, userId, userName, reactionType })
        });
    ...
    async unlike(facultyId) {
        ...
        const response = await fetch(`${API_BASE_URL}/social/like`, {
            ...
            body: JSON.stringify({ facultyId, userId })
        });
    ...
    async getLikes(facultyId) {
        const response = await fetch(`${API_BASE_URL}/social/likes/${facultyId}`);
    ...
    async hasUserLiked(facultyId) {
        const userId = getUserId();
        const data = await this.getLikes(facultyId);
        return data.likes.some(like => like.userId === userId);
    },
    ...
    async addComment(facultyId, content) {
        ...
        const userEmail = localStorage.getItem('social_user_email') || '';
        ...
        const response = await fetch(`${API_BASE_URL}/social/comment`, {
            ...
            body: JSON.stringify({ facultyId, userId, userName, userEmail, content })
        });
    ...
    async getComments(facultyId, limit = 50, offset = 0) {
        const response = await fetch(
            `${API_BASE_URL}/social/comments/${facultyId}?limit=${limit}&offset=${offset}`
        );
    ...
    async deleteComment(commentId) {
        ...
        const response = await fetch(`${API_BASE_URL}/social/comment/${commentId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    async follow(facultyId) {
        ...
        const response = await fetch(`${API_BASE_URL}/social/follow`, {
            ...
            body: JSON.stringify({ facultyId, userId, userName, userEmail })
        });
    ...
    async getFollowers(facultyId) {
        const response = await fetch(`${API_BASE_URL}/social/followers/${facultyId}`);
    ...
    async isFollowing(facultyId) {
        const userId = getUserId();
        const data = await this.getFollowers(facultyId);
        return data.followers.some(follower => follower.userId === userId);
    },
    ...
    async getStats(facultyId) {
        const response = await fetch(`${API_BASE_URL}/social/stats/${facultyId}`);
...
const SocialUI = {
    ...
    createSocialButtons(facultyId, options = {}) {
        ...
        container.className = 'social-actions flex items-center gap-3 mt-4';
        container.innerHTML = `
            <button 
                onclick="handleLikeClick('${facultyId}')" 
                id="like-btn-${facultyId}"
                class="social-btn like-btn flex items-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-all duration-300 transform hover:scale-105"
            ...
            <button 
                ...
                class="social-btn comment-btn flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-all duration-300 transform hover:scale-105"
            ...
            <button 
                ...
                class="social-btn follow-btn flex items-center gap-2 px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-lg transition-all duration-300 transform hover:scale-105"
            ...
            <button 
                ...
                class="social-btn share-btn flex items-center gap-2 px-4 py-2 bg-pink-500 hover:bg-pink-600 text-white rounded-lg transition-all duration-300 transform hover:scale-105"
    ...
    renderComment(comment, canDelete = false) {
        ...
        return `
            <div class="comment-item bg-gray-50 rounded-lg p-4 border-l-4 border-green-500" data-comment-id="${comment.id}">
                ...
                <p class="text-gray-700 leading-relaxed">${this.escapeHtml(comment.content)}</p>
            </div>
        `;
    },
 ...
 */
async function initSocialFeatures(facultyId, containerSelector = null) {
    try {
        // Get social stats
        ...
        if (likeBtn && hasLiked) {
            ...
            likeBtn.classList.add('bg-blue-600');
        }
...
async function handleLikeClick(facultyId) {
    ...
    try {
        ...
        if (hasLiked) {
            ...
            likeBtn.classList.remove('liked', 'bg-blue-600');
...
async function loadComments(facultyId) {
    ...
    try {
        const data = await SocialAPI.getComments(facultyId);
        
        if (data.comments.length === 0) {
            commentsList.innerHTML = '<p class="text-gray-500 text-center py-4">No comments yet. Be the first to comment!</p>';
        } else {
            commentsList.innerHTML = data.comments.map(comment => 
                SocialUI.renderComment(comment, comment.userId === userId)
            ).join('');
        }
    } catch (error) {
        console.error('Error loading comments:', error);
        commentsList.innerHTML = '<p class="text-red-500 text-center py-4">Failed to load comments. Please try again.</p>';
    }
}
...
async function submitComment(facultyId) {
    const commentInput = document.getElementById(`comment-input-${facultyId}`);
    ...
    try {
        ...
        const stats = await SocialAPI.getStats(facultyId);
        const commentCount = document.getElementById(`comment-count-${facultyId}`);
        if (commentCount) commentCount.textContent = stats.commentsCount;
...
async function handleFollowClick(facultyId) {
    ...
    try {
        ...
        if (isFollowing) {
            ...
            followBtn.classList.remove('following', 'bg-gray-500', 'hover:bg-gray-600');
        ...
        alert('Failed to process your follow request. Please try again.');
    }
}
 ...
 */
function handleShareClick(facultyId) {
...
function showShareModal(url, title) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-2xl p-8 max-w-md w-full mx-4 shadow-2xl">
            <h3 class="text-2xl font-bold text-gray-900 mb-6">Share this profile</h3>
            <div class="grid grid-cols-2 gap-4 mb-6">
                <a href="${shareLinks.facebook}" target="_blank" 
                   class="flex items-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all">
                ...
                <a href="${shareLinks.twitter}" target="_blank"
                   class="flex items-center gap-2 px-4 py-3 bg-sky-500 hover:bg-sky-600 text-white rounded-lg transition-all">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a href="${shareLinks.linkedin}" target="_blank"
                   class="flex items-center gap-2 px-4 py-3 bg-blue-700 hover:bg-blue-800 text-white rounded-lg transition-all">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                <a href="${shareLinks.whatsapp}" target="_blank"
                   class="flex items-center gap-2 px-4 py-3 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-all">
                    <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-2">Or copy link:</label>
                <div class="flex gap-2">
                    <input type="text" value="${url}" readonly
                           class="flex-1 px-4 py-2 border-2 border-gray-300 rounded-lg bg-gray-50"
                           id="share-url-input">
                    <button onclick="copyShareUrl()"
                            class="px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white rounded-lg transition-all">
                        Copy
                    </button>
                </div>
            </div>
            <button onclick="closeShareModal()"
                    class="w-full px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg transition-all">
                Close
            </button>
        </div>
    `;
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { SocialAPI, SocialUI, initSocialFeatures };
}
